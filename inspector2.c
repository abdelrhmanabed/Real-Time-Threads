...
void cleanup() {
    if (shmdt(shmptr_stats) == -1) {
        // perror("shmdt -- inspector2 -- detach");
    }
    if (shmctl(shmid_stats, IPC_RMID, NULL) == -1) {
        // perror("shmctl -- inspector2 -- remove");
    }
    if (semctl(semid_stats, 0, IPC_RMID) == -1) {
        // perror("semctl -- inspector2 -- remove");
    }
}
...
void moveProcessedFile(FileMetadata *file);
void initializeIPCS();
void lockSemStats();
void unlockSemStats();
...
int main(int argc, char *argv[]) {
    ...
    while (1){
        if (shmptr_stats->totalProcessed == 0){
            sleep(1);
            continue;
        }
        ...
        for (int i = 0; i < shmptr_stats->totalGenerated; i++) {
            ...
            if (file->status == STATUS_PROCESSED && difftime(time(NULL), file->generateTime) > (double) shmptr_config->processedAge) {
                // move to backup dir
...
void moveProcessedFile(FileMetadata *file){
    char backupPath[256];
    snprintf(backupPath, sizeof(backupPath), "%s/%d.csv", BACKUP_DIR, file->serial_number);

    if (rename(file->fileName, backupPath) == -1) { // move the file to the backup directory
        perror("Error moving file to UnProcessed directory");
    } else {
        printf("Inspector2: Moved file %s to %s\n", file->fileName, backupPath);
        strcpy(file->fileName, backupPath);
        file->status = STATUS_BACKEDUP;
    }

    lockSemStats();
    shmptr_stats->totalBackedup++;
    unlockSemStats();
}
...
void initializeIPCS(){
    ...
    if ((shmid_stats = shmget((int) key, sizeof(SharedStats), 0777)) != -1) {
        ...
        if ((shmptr_stats = (SharedStats *) shmat(shmid_stats, 0, 0)) == (SharedStats *) -1) {
            perror("shmptr -- inspector2 -- attach");
            exit(1);
        }
    } else {
        perror("shmid -- inspector2 -- creation");
        exit(2);
    }

    if ((semid_stats = semget(key, 1, 0777)) == -1) {
        perror("semget -- inspector2 -- access");
        exit(1);
    }
    ...
    if ((semid_metaData = semget((int) key, 1, 0777)) == -1) {
        perror("semid -- inspector2 -- initialization");
        exit(3);
    }
}
...
void lockSemStats(){
    if(semop(semid_stats, &sem_lock, 1) == -1){	//lock semaphore to read data
        perror("semop -- inspector2 -- lock");//handle error locking semaphore
        exit(5);
    }
}
...
void unlockSemStats(){
    if(semop(semid_stats, &sem_unlock, 1) == -1){	//unlock semaphore
        perror("semop -- inspector2 -- unlock");//handle error unlocking semaphore
        exit(5);
    }
}
